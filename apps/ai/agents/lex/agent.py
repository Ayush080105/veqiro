...
class LexAgent(BaseAgent):
    ...
    async def build_system_prompt(
        ...
        lex_specific += _greeting + (
            ...
            "\n## Connected Tools (e.g. Google Drive)\n"
            "If the user has connected a document source like Google Drive, you may have extra tools "
            "available beyond the six listed above — use their find/search and download/read actions "
            "to locate and read a specific file the user references, read-only. Never use a connected "
            "source's create, edit, delete, or sharing/permission tools unless the user explicitly asks "
            "you to change or share something — your job is to read and analyze, not modify their files.\n"
