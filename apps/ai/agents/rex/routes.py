...
def _render_board_html(
    ...
    metric_rows = "".join(
        f"<tr>"
        f"<td style='padding:6px 12px;border-bottom:1px solid #eee;font-weight:500;'>{k}</td>"
        f"<td style='padding:6px 12px;border-bottom:1px solid #eee;text-align:right;font-family:monospace'>{v}</td>"
        f"</tr>"
        for k, v in (metrics or {}).items()
    )
    ...
    def sec(title, body):
        return (
            ...
            f"letter-spacing:2px;color:#666;border-bottom:2px solid #111;"
            f"padding-bottom:4px;margin:0 0 10px 0;'>{title}</h2>"
            f"<div style='font-size:14px;line-height:1.6;"
            f"color:#222;white-space:pre-wrap;'>{body}</div>"
            f"</section>"
        )
    ...
    metrics_section = (
        ...
        "letter-spacing:2px;color:#666;border-bottom:2px solid #111;"
    ...
    return (
        ...
        "<header style='border-bottom:3px solid #111;"
        "padding-bottom:12px;margin-bottom:24px;'>"

        f"<div style='font-family:monospace;font-size:11px;"
        f"text-transform:uppercase;letter-spacing:2px;color:#666;'>"
        ...
        f"<footer style='margin-top:32px;font-family:monospace;"
        f"font-size:10px;color:#888;text-align:center;'>"
        f"Generated by REX · {datetime.utcnow().strftime('%Y-%m-%d')}"
        "</footer>"
