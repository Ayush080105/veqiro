...
class RexAgent(BaseAgent):
    ...
    def get_tool_instructions(self) -> str:
        return (
            ...
            "'Data' means either the user typed it in this conversation, OR you fetched it yourself via a "
            "connected tool (see Connected Tools below) — both count as real. What's still forbidden is "
            "guessing a number from memory or general knowledge and presenting it as this company's data. "
            ...
            "## Tool Usage Rules\n"
            "Only call tools with real data — either typed by the user, or fetched via a connected tool.\n"
            ...
            "Never call any tool with made-up values.\n\n"
            "## Connected Tools\n"
            "You may have MCP tools for connected data sources — each one's LLM-facing name is prefixed "
            ...
            "**Revenue & payments** — `mcp_stripe_*` (core input: MRR/churn/revenue), "
            "`mcp_quickbooks-online_*` (P&L/balance sheet), `mcp_razorpay_*` (India-market revenue).\n"
            "**Marketing & CRM (shared with Maya)** — `mcp_google-ads_*`, `mcp_hubspot-marketing_*`, "
            "`mcp_mailchimp_*`, `mcp_zoho_*` — your angle is ROAS/CAC/campaign performance and CRM "
            "pipeline data; Maya uses the same connection to draft creative off the same source.\n"
            "**Product & web analytics** — `mcp_google-analytics-4_*`, `mcp_posthog_*` — traffic/usage "
            "trends feeding forecasts.\n"
            "**Spreadsheets** — `mcp_google-sheets_*`, `mcp_microsoft-excel_*` — lightweight/ad-hoc "
            "dataset source when nothing else is connected.\n"
            "**Databases** — `mcp_supabase_*`, `mcp_clickhouse_*` — query for warehouse-scale metrics.\n"
            "**Project management** — `mcp_asana_*`, `mcp_linear_*`, `mcp_clickup_*` — project-status "
            "signals for executive briefings.\n"
            "Use their read/query/list-shaped actions to fetch real numbers, then feed the result into "
            "your normal analysis tools above. NEVER use a connected tool's write, delete, or mutating "
            "action — no INSERT/UPDATE/DELETE/DROP/ALTER/TRUNCATE against a database, no refund/charge/"
            "payout/subscription-change against a payment processor, no creating/editing/deleting a "
            "ticket in a project tracker, no editing a spreadsheet cell — unless the user has explicitly "
            "asked for that exact action in this conversation. When in doubt about whether an action "
            "reads or writes, treat it as a write and ask first.\n\n"
