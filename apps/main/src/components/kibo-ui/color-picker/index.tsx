...
export const ColorPicker = ({
  ...
  // VEQIRO PATCH: keep onChange in a ref so the notify-parent effect below
  // doesn't refire whenever the parent passes a fresh inline arrow. Without
  // this, calling onChange triggers a parent re-render → new fn ref →
  // re-fires the effect → infinite loop (#max-update-depth).
  const onChangeRef = useRef(onChange);
  useEffect(() => {
    onChangeRef.current = onChange;
  });

  // VEQIRO PATCH: skip the first-mount fire. The picker boots with synthetic
  // initial state and would otherwise call onChange with that state on mount,
  // dirtying the parent form before the user has touched anything.
...
export const ColorPickerOutput = ({
  ...
  return (
    <Select onValueChange={(v) => setMode(v ?? "hex")} value={mode}>
